...
# --- ⚡ MAGIC LINK AUTO-LOGIN (PRIORITY 1) ---
# This runs BEFORE anything else. If it sees a code, it forces login.
query_params = st.query_params
magic_code = query_params.get("code")

if magic_code:
    # Check if the code is valid (Hardcoded 'CLIENT-9921' for your test)
    if magic_code == "CLIENT-9921" or magic_code in cs.ACCESS_CODES:
        if not st.session_state.authenticated:
            st.session_state.authenticated = True
            st.session_state.user_mode = "client"
            st.success("🔓 Magic Link Authorized. Logging in...")
            time.sleep(1) # Tiny pause for effect
            st.rerun()

# --- 🛡️ SIDEBAR (RESET & LOGIN) ---
with st.sidebar:
    # 🔴 RESET BUTTON (FOR TESTING)
    if st.button("🔄 RESET / LOGOUT", type="primary"):
        st.session_state.clear()
        st.rerun()
    st.caption("Click above if you get stuck.")
    st.divider()
    ...
    with tab_dispatch:
        ...
        with st.form("dispatch_form"):
            ...
            if submitted and selected_forms:
                base_url = "https://formflux.streamlit.app" # YOUR URL
                query_string = "&".join([f"form={urllib.parse.quote(f)}" for f in selected_forms])
                # WE ADD THE CODE TO THE LINK
...
# =========================================================
    ...
    if not st.session_state.authenticated:
        ...
        with c2:
            ...
            # If magic link failed, they see this manual entry
